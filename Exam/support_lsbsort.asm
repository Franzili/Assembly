...
check_2K_up:
	;; data[i+1] (only key)
	mov	edx, [sorted_data + 8 * rcx]
	cmp	edx, ebx
	;; data[i+1] < data[i]: Fehler
	jl	check_2K_error
	;; weiter
...
check_2K_order:
	;; Element laden
	mov 	rdx, [sorted_data + 8 * rcx]
	;; Payload (oberes Doppelwort) extrahieren
	mov	rdi, rdx
	shr	rdi, 32
	;; rdi als Index verwenden und originale Daten laden
	mov	rbx, [data + 8 * rdi]
	;; Vergleich
	cmp	rbx, rdx
	;; Daten löschen
	mov	qword [data + 8 * rdi], 0
	jne	check_2K_error
	;; weiter
	inc	rcx
	cmp	rcx, ELEMS
...
%ifdef SWAPERROR
	...
	mov	rax, [sorted_data + 8 * (ELEMS/2)]
	xchg	[sorted_data + 8 * (ELEMS/2) + 8], rax
	mov	[sorted_data + 8 * (ELEMS/2)], rax
%endif
...
%ifdef COPYERROR
	...
	call	write_str_stdout
	mov	rax, [sorted_data + 8 * (ELEMS/2)]
	mov	[sorted_data + 8 * (ELEMS/2) + 8], eax
%endif
